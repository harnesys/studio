...
System tools: `ask_user` (`prompt` + optional `options[{id,label}]`), `map` (`items` array → `$state.mapItems` → `control:map`), `wait` (`delayMs` → `$state.waitUntilMs` → `control:wait`) live in the host registry but are usable only when named in the agent's `tools` (closed world). `load_tools` is always in the run registry and `load_skill`/`Skill` join it whenever the agent has skills; the resolver adds them after the agent filter, so name them in node `tools` only. There is no `knowledge_upsert`, no `semantic_upsert`, no `semantic_search`. `edit_file` args are `path`, `old_string`, `new_string` (unique match required), not camelCase. ReAct presets that expose map/wait must route `act` → `map` when `exists($state.mapItems)` and `act` → `wait` when `exists($state.waitUntilMs)`, with a map body ending in `control:yield`. Prefer `wait` for mid-run sleep; `schedule_set` for recurring / next-session fires after the run ends.
