...
Поведение `anchor` по исходникам примитива (зафиксировать в ожиданиях ручной проверки): после отправки вопрос паркуется к верху с peek; пока терн короче вьюпорта, spacer доливает резерв и край считается живым (`!end`, кнопка спрятана). Живая проверка 2026-09-15 подтвердила: когда контент терна переполняет вьюпорт, примитив переключается на `following-bottom` и догоняет хвост (формулировка спеки «follow подхватывает, когда стрим заполняет вьюпорт» работает). Колесо/тач выше края — detach: вьюпорт стоит, растёт unseen, возврат кнопкой или доскроллом до края включает follow.
