...
- Валидация скиллов прецедент: `application/agents/create-agent.use-case.ts:124-131` — dep `workspaceCatalog.listSkills: ListWorkspaceSkillsInput`, `execute({workspaceId})` → `listed.skills[].name`, `assertAllKnown(unknown,'skill')`. Поле allowlist — `agent.skills?: string[]` (`shared/src/agent.ts:44-45`: пусто = все).
- Каталог-семантика фильтра: `packages/harnesys/src/application/skills/skills-catalog.ts` `filterSkills` (пусто = все). Клиентский хеллер: `shared/api/workspaces.ts:87-109` (`GET /api/workspaces/:id/skills`), тип `WorkspaceSkill` (`shared/src/workspace-config.ts:7`).
...
- [ ] **Step 1:** `skill-source.ts`: `useQuery(workspaceSkillsQuery(workspaceId))` + фильтр по `agent?.skills` (пусто = все), `useMemo` → `SkillOption[]`. Хук живёт в `widgets/chat-composer` (read-only данные, не фича: запись нет).
