...
export function PluginsPane() {
  ...
  const install = useMutation({
    mutationFn: (body: InstallPluginRequest) => installPlugin(body),
      ...
      <div className="flex h-8 items-center gap-1">
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <Button
            ...
            onClick={() => {
              void openInstallPluginDialog().then((draft) => {
                if (!draft) {
                  return;
                }
                install.mutate(draft);
              });
            }}
          >
            <PlusIcon />
        ...
        (items.length === 0 ? (
          <Empty className="min-h-0 border-0 py-8">
            <EmptyHeader>
              ...
              <EmptyDescription>
                Install from `owner/repo` (for example `obra/superpowers`) or a git URL.
              </EmptyDescription>
            </EmptyHeader>
          </Empty>
        ) : (
